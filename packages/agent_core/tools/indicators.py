...
def calculate_technical_indicators(ohlcv_json: str) -> str:
    ...
    result: dict[str, Any] = {
        ...
        "data_points": len(df),
        # RSI
        "rsi": _safe_float(rsi.iloc[-1]) if rsi is not None else None,
        # MACD
        "macd_line": None,
        "macd_signal": None,
        "macd_histogram": None,
        ...
        "atr": None,
        # EMAs
        "ema_20": _safe_float(ema20.iloc[-1]) if ema20 is not None else None,
        "ema_50": _safe_float(ema50.iloc[-1]) if ema50 is not None else None,
        "ema_200": _safe_float(ema200.iloc[-1]) if ema200 is not None else None,
    }
    ...
    if macd is not None:
        for col in macd.columns:
            ...
            if "macd_" in col_lower and "macds" not in col_lower and "macdh" not in col_lower:
                result["macd_line"] = _safe_float(macd[col].iloc[-1])
            elif "macds" in col_lower:
                result["macd_signal"] = _safe_float(macd[col].iloc[-1])
            elif "macdh" in col_lower:
                result["macd_histogram"] = _safe_float(macd[col].iloc[-1])

    if bb is not None:
        for col in bb.columns:
            col_lower = col.lower()
            if col_lower.startswith("bbu"):
                result["bb_upper"] = _safe_float(bb[col].iloc[-1])
            elif col_lower.startswith("bbm"):
                result["bb_middle"] = _safe_float(bb[col].iloc[-1])
            elif col_lower.startswith("bbl"):
                result["bb_lower"] = _safe_float(bb[col].iloc[-1])
            elif col_lower.startswith("bbb"):
                result["bb_bandwidth"] = _safe_float(bb[col].iloc[-1])

    if adx is not None:
        for col in adx.columns:
            col_lower = col.lower()
            if col_lower.startswith("adx_"):
                result["adx"] = _safe_float(adx[col].iloc[-1])
            elif "dmp" in col_lower:
                result["adx_pos_di"] = _safe_float(adx[col].iloc[-1])
            elif "dmn" in col_lower:
                result["adx_neg_di"] = _safe_float(adx[col].iloc[-1])
